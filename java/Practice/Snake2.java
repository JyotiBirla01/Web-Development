import java.io.*; // used to allow user to input data
 ...
 class Snakes_and_Ladders {// start class
     ...
     * startGame method:
     * 
     * This method is responsible for organizing the game, asking the user to
     * continue playing,
    ...
    {// start startGame method
        ...
        {// start playAgain While
            ...
            System.out.println("\t\t\t\t\t|\tThe Computer Rolled a " + compRoll + "\t|"); // print the roll the computer
                                                                                          // got
            System.out.println("\t\t\t\t\t------------------------------------------");

            // hold the user's last position for switching back if current position was
            // greater than 100
            userPosition = userPosition + userRoll;

            // hold the computer's last position for switching back if current position was
            // greater than 100
            compPosition = compPosition + compRoll;

            // check to see if user landed on top of a snake or at the bottom of a ladder
            // give getP parameters to work with, and recieve a final value which can be
            // printed out
            ...
            System.out.println("\t\t\t*************************************************************************");
            System.out.println("\t\t\t*\t\tYou are currently on square " + userPosition + "\t\t\t*"); // print out the
                                                                                                      // user's current
                                                                                                      // location
            System.out.println("\t\t\t*\t\tThe Computer is currently on square " + compPosition + "\t\t*"); // print out
                                                                                                            // the
                                                                                                            // computer's
                                                                                                            // current
                                                                                                            // location
        ...
        return playAgain; // returns prameter "playAgain" to main: if the user wants to play the game
                          // again
     ...
     * ladders value.
     */
    public static int getP(int userPosition, int userRoll, int snakesLaddersArray[]) throws IOException // recieves two
                                                                                                        // parameter
                                                                                                        // from
                                                                                                        // startGame
    {// start getP
        ...
        if (userPosition < 0 || userPosition > 112) // This is ab ERROR TRAP to catch any unwanted system errors that
                                                    // may occur by chance
        {
        ...
        return userPosition; // return the final position to starGame: this position had gone through all
                             // checks and test and can be displayed on screen
    }// end getP
    ...
    public static int compgetP(int compPosition, int compRoll, int snakesLaddersArray[], int userPosition)
            throws IOException {// start compgetP
        ...
        if (compPosition < 0 || compPosition > 112) // ERROR TRAP to catch any unwanted system errors that may occur by
                                                    // chance
        {
        ...
        else if (compPosition > 100) // checks if computers's location if greater then a 100
        {
            ...
            System.out.println("THE COMPUTER CAN'T JUMP!!! He must land on a 100"); // give message that the computer
                                                                                    // cant jump
        ...
        return compPosition; // return the final position to starGame: this position had gone through all
                             // checks and test and can be displayed on screen
    } // end compgetPy
}// end class
